        ...
        */
        ...
        for (int k = 1; k < num_m; k++)
        {
            ...
            */
            ...
            cv::matchTemplate(l_rim, l_sim(cv::Range(l_sim.rows / 10, (9 * l_sim.rows) / 10), cv::Range(l_sim.cols / 10, (9 * l_sim.cols) / 10)), l_tim, cv::TM_CCOEFF_NORMED);
            ...
            cv::matchTemplate(h_rim, h_sim(cv::Range(h_sim.rows / 10, (9 * h_sim.rows) / 10), cv::Range(h_sim.cols / 10, (9 * h_sim.cols) / 10)), h_tim, cv::TM_CCOEFF_NORMED);
            cv::minMaxLoc(h_tim, &min_h, &max_h, &p_min_h, &p_max_h);

            cvt_HOG(sim, sh_sim);
            cv::matchTemplate(sh_rim, sh_sim(cv::Range(sh_sim.rows / 10, (9 * sh_sim.rows) / 10), cv::Range(sh_sim.cols / 10, (9 * sh_sim.cols) / 10)), sh_tim, cv::TM_CCOEFF_NORMED);
